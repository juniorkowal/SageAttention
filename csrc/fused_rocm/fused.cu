...
#include "../common_rocm/reduction_utils.h"
...
//   static_assert(std::is_same<T, half>::value || std::is_same<T, nv_bfloat16>::value, "Only half and bfloat16 are supported");
  ...
  bx * CTA_SIZE + thread_id % num_threads_per_cta * pack_size + thread_id / num_threads_per_cta * stride_d_output;
  ...
  uint32_t smem_load_row = thread_id / num_threads_per_token;
